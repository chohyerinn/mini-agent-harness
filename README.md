...
평가 도구는 그 자체가 맞다는 보장이 필요합니다. `tests/`는 harness 흐름뿐 아니라 **채점 로직 자체**를 검증합니다 — 정답을 아는 케이스에서 solve rate·pass@k·McNemar가 기대값대로 나오는지, 변조 탐지가 실제로 잡는지. 즉 "에이전트를 채점하는 코드"를 다시 채점하는 셈입니다.
